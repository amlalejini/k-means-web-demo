...
public:
  ...
  bool DoToggleRun() {
    ...
    if (anim.GetActive()) {
      ...
      run_but.SetLabel("Pause");
      step_but.SetDisabled(true);
    } else {
      run_but.SetLabel("Run");
      step_but.SetDisabled(false);
    }
    return true;
  }
  ...
  void Draw() {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
      ...
      canvas.Circle(points[i].GetCenter(), points[i].GetRadius(), color);
    }
    // Draw centroids.
    for (size_t i = 0; i < centroids.size(); ++i) {
      auto & centroid = centroids[i];
      canvas.Circle(centroid.GetCenterX(), centroid.GetCenterY(), centroid.GetRadius() * 2, "grey");
      canvas.Circle(centroid.GetCenter(), centroid.GetRadius(), color_map[i]);
    }
  }
  ...
  void ClusterSingleStep() {
    ...
    if (cluster_iteration == 0) {
      ...
      // --- This code would assign random cluster membership ---
      // for (size_t i = 0; i < cluster_ids.size(); ++i) { cluster_ids[i] = i % num_bins; }
      // emp::Shuffle(random, cluster_ids);
      // --- ---
      // Shuffle points, pick K random to be centroids.
      emp::vector<size_t> point_ids(points.size());
      std::iota(
        point_ids.begin(),
        point_ids.end(),
        0
      );
      emp::Shuffle(random, point_ids);
      emp_assert(centroids.size() <= point_ids.size());
      for (size_t i = 0; i < centroids.size(); ++i) {
        auto & p = points[point_ids[i]];
        centroids[i].Set(
          p.GetCenterX(),
          p.GetCenterY(),
          point_radius+1
